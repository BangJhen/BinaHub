"""
BinaHub Backend - All LLM prompts and prompt-builder functions.
Keeping prompts in one file makes iteration and translation easy.
"""
...
def build_quick_prompt(journal_text: str, worker_context: dict | None = None) -> str:
    """Prompt untuk fast-path analysis (tanpa riwayat RAG)."""
    ctx = ""
    if worker_context:
        ctx = f"""
KONTEKS INDIVIDU:
...
def build_rag_prompt(journal_text: str, context: dict, worker_context: dict | None = None) -> str:
    """Prompt untuk full RAG-enhanced analysis dengan konteks historis 7 hari."""
...
def build_question_prompt(worker_context: dict | None = None) -> str:
    ctx = ""
    if worker_context:
        ctx = f"""
KONTEKS PEKERJA:
